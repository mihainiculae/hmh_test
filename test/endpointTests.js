        ...
        it("Info for the requested cities is returned", function(done) {
            ...
            request.doRequest(requestObject, function(e, r) {
                ...
                resAsJson.location_suggestions.forEach(location => {
                    if (!city_ids.includes(location.id.toString()))
                        notFound.push(location.name);
                });
                assert.equal(
                    notFound.length,
                    0,
                    `${notFound} was not found in the response!`
                );
                done(checkOkCitiesResponse(r));
            });
        });
    ...
    describe("When requesting for cities specifying a max count", function() {
        it("the response should not surpass that max count", function(done) {
            ...
            request.doRequest(requestObject, function(e, r) {
                ...
                var resAsJson = JSON.parse(r.responseBuffer);
                assert.ok(
                    maxCities === resAsJson.location_suggestions.length,
                    `The number of cities returned did not match what we sent. got ${
                        resAsJson.location_suggestions.length
                    }`
                );
                done(checkOkCitiesResponse(r));
            });
        ...
        it("The response should have status code 403", function(done) {
            ...
            request.doRequest(requestObject, function(e, r) {
                ...
                assert.equal(
                    r.statusCode,
                    403,
                    `The status code was not 403, but was: ${r.statusCode}`
                );
                done();
            });
        });
...
function checkOkCitiesResponse(response) {
    ...
    var errorsFound = [];
    if (resAsJson.status !== "success")
        errorsFound.push(`The query was not a success`);
    if (resAsJson.location_suggestions.length === 0)
        errorsFound.push(`No results were returned!`);
    var undefinedKeys = returnUndefinedKeys(resAsJson, expectedObjectFields);
    if (undefinedKeys.length !== 0)
        errorsFound.push(
            `Some keys in the response object were undefined: ${undefinedKeys} were undefined`
        );
    resAsJson.location_suggestions.forEach(suggestion => {
        ...
        if (undefKeys.length !== 0)
            errorsFound.push(
                `Some keys in a location suggestion object were undefined: ${undefKeys} were undefined`
            );
    });
